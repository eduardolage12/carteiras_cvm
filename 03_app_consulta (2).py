...
def _get_base_dir() -> str:
    """Retorna o diretorio base do repositorio de forma robusta."""
    # Streamlit Cloud: o repositorio fica em /mount/src/<nome_repo>
    for caminho in [
...
def _importar_script(nome_arquivo: str):
    """Importa script Python pelo caminho do arquivo."""
...
def inicializar_base() -> None:
    """Baixa e processa os dados automaticamente se o parquet nao existir."""
    parquet_path = os.path.join(_get_base_dir(), "dados_processados", "carteiras_consolidadas.parquet")
    if os.path.exists(parquet_path):
        return

    st.info("Base de dados nao encontrada. Iniciando download e processamento automatico...")

    try:
        baixar = _importar_script("01_baixar_dados.py")
        processar = _importar_script("02_processar_dados.py")
    except Exception as e:
        st.error(f"Erro ao importar scripts auxiliares: {e}")
        st.code(traceback.format_exc())
        st.stop()

    # Etapa 1: Baixar dados
    st.write("⏳ Baixando dados da CVM... (pode levar alguns minutos)")
    progresso = st.empty()
    try:
        # Mudar para o diretorio do repositorio para os scripts funcionarem
        os.chdir(_get_base_dir())
        meses = baixar.baixar_carteiras()
        if not meses:
            st.error("Nao foi possivel baixar os dados da CVM.")
            st.stop()
        progresso.success(f"Download concluido: {len(meses)} meses baixados.")
    except Exception as e:
    ...
    try:
        ...
        processar.processar_dados()
        progresso2.success("Processamento concluido!")
    except Exception as e:
        st.error(f"Erro no processamento: {e}")
        st.code(traceback.format_exc())
        st.stop()

    st.success("Base de dados pronta! Recarregue a pagina.")
    st.stop()
...
def carregar_dados() -> pd.DataFrame:
    base_dir = _get_base_dir()
    parquet_path = os.path.join(base_dir, "dados_processados", "carteiras_consolidadas.parquet")
...
try:
    ...
    st.error("Base nao encontrada. Recarregue a pagina para tentar novamente.")
    st.stop()
