...
def _importar_script(nome_arquivo: str):
    """Importa script Python pelo caminho do arquivo (suporta nomes com numeros)."""
...
def inicializar_base() -> None:
    ...
    if os.path.exists("dados_processados/carteiras_consolidadas.parquet"):
        return

    st.info("Base de dados nao encontrada. Iniciando download e processamento automatico...")

    # Importar funcoes dos scripts auxiliares
    try:
    ...
    with st.status("Baixando dados da CVM... (pode levar alguns minutos)", expanded=True) as status:
        try:
            meses = baixar.baixar_carteiras()
            if not meses:
                st.error("Nao foi possivel baixar os dados da CVM.")
                st.stop()
            status.update(label=f"Download concluido: {len(meses)} meses baixados.", state="complete")
        except Exception as e:
    ...
    with st.status("Processando dados...", expanded=True) as status:
        try:
            ...
            status.update(label="Processamento concluido!", state="complete")
        except Exception as e:
            st.error(f"Erro no processamento: {e}")
            st.stop()

    st.success("Base de dados pronta! Carregando o app...")
    st.rerun()
